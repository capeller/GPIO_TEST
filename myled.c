...
MODULE_DESCRIPTION("A simple driver for controlling RasPi GPIO - Change to RasPi3");
MODULE_LICENSE("GPL");
MODULE_VERSION("0.2");
...
static ssize_t sw_read(struct file *flip, char *buf, size_t count,
                       loff_t *pos) {
  unsigned int ret = 0;
  int len;
  int index;
...
static ssize_t sushi_read(struct file *flip, char *buf, size_t count,
                          loff_t *pos) {
  int size = 0;
  char sushi[] = {0xF0, 0x9F, 0x8D, 0xA3, 0x0A}; // sushi
  if (copy_to_user((void *)buf + size, (const char *)sushi, sizeof(sushi))) {
